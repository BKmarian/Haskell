...
charToElem c = if (isDigit c == False) then error "Eroare" else prime!!(digitToInt c)
...
strToInt :: String -> Int
strToInt str = if (length str == 1) then digitToInt(head str) else 10 * digitToInt(head str) + strToInt(tail str)
...
enc :: String -> String
enc str = concat [charToElem s | s <- str]
...
encmsg str = enc (filter (\x -> isDigit x) (concat str))
